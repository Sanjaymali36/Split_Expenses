...
def CreateAccount(request): #code for create user profile/account
    return render(request,'Details.html')
def savedetails(request):  #code for save user profile/account
...
def Storedetails(request):#code for add user expenses
...
def calculate_expense(request): #code for Calculate expenses
    global sum  # Declear sum as global variable because sum need for another function  i.e spit_expenses
    uid=request.POST["u1"]
    result=ExpenseType.objects.get(User_ID=uid) #get object
...
def split_expense(request):#code for slit expenses individual
